...
#define SRC_PATH std::string("../img")
...
int main(){
    cv::Mat img = cv::imread("../img/0.bmp",0);
    Canny canny = Canny(3,img,5,2);
    canny.Canny_edge_detect();
    cv::imwrite("canny_img.bmp",canny.img_);
    cv::Mat opencv_canny;
    cv::Canny(img,opencv_canny,100,150);
    cv::imwrite("opencv_canny.jpg",opencv_canny);
